...
describe('CostPerKgChart', () => {
  beforeEach(() => { setActivePinia(createPinia()) })

  it('renders SVG element', () => {
    const wrapper = mount(CostPerKgChart, {
  ...
  it('has accessible container', () => {
    const wrapper = mount(CostPerKgChart, {
      props: { data: [{ rocket_id: 'r1', rocket_name: 'F9', cost_per_launch: 50000000, launches: 1, total_spend: 50000000, payload_kg_leo: 22800, cost_per_kg_leo: 2193 }] },
    })
  ...
  it('does not crash with empty data', () => {
    const wrapper = mount(CostPerKgChart, { props: { data: [] } })
    expect(wrapper.find('svg').exists()).toBe(true)
  })
})
